 ...
 <div  id="table_schedule">
  <center><h3>View Orders Process </h3></center>
	<?php
	  ...
	  $query = "SELECT *FROM orders where status='Process'";
	  $result = mysqli_query($conn,$query);
	  static $row_num = 0;

	  if(mysqli_num_rows($result)>0){

		echo'<center><table><tr><th>Order ID</th><th>Depature</th><th>Arrival</th><th>Sender Name</th><th>Sender Phone</th><th>Receiver Name</th><th>Receiver Phone</th><th>Container</th><th>Function</th></tr>';
		  // output data of each row
		  while($row = mysqli_fetch_assoc($result)){
			  $row_num++;
			  echo'<tr class="thead-light"><td>'.$row["Order_ID"].'</td><td>'.$row["Depature"].'</td><td>'.$row["Destination"].'</td><td>'.$row["Sender_name"].'</td><td>'.$row["Sender_Phone"].'</td><td>'.$row["Receiver_name"].'</td><td>'.$row["Receiver_Phone"].'</td><td>'.$row["container_model"].'</td><td><a  class="accept_btn" href="proceed.php?id='.$row["container_model"].'">Proceed</a> <a  class="delete_btn" href="deny.php?id='.$row["container_model"].'">Deny</a></td></tr>';}
		  echo'</center></table>';
